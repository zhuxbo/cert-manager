...
// 插槽 props：order（含 id/user_id）、cert（含 status / encryption_alg）
...
async function pushAll() {
  if (!props.order?.id || !props.order?.user_id) {
    ElMessage.warning("订单缺少用户信息，无法推送");
    return;
  }
...
async function loadRecords() {
  ...
  try {
    ...
    // 默认去噪只显终态。用 1（非 JS 布尔 true）：qs 序列化 true→"true" 不被 Laravel boolean 规则接受会 422。
