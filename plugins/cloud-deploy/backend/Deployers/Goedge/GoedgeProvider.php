...
/**
 * GoEdge（自建 CDN 面板）凭证 schema。
 *
 * 对齐 certimate AccessConfigForGoEdge：serverUrl + apiRole（user/admin，登录时作 type）+ accessKeyId
 * + accessKey（用 AK 换登录态 token）+ allow_insecure（自签证书自建面板常见，允许跳过 TLS 校验）。
 */
class GoedgeProvider implements ProviderInterface
{
