 ...
 *
 * 9 端点：scm（SCM 证书管理纯托管）/ cdn（CDN 全局绑证书）/ elb（弹性负载均衡监听器）/ waf（云模式防护域名）/
 * live（视频直播域名）/ vod（视频点播域名）/ obs（对象存储自定义域名）/ apig（API 网关替换证书）/ aad（DDoS 高防按域名设证书）。
 *
...
return function (Registry $registry): void {
    ...
    $registry->registerDeployer('huaweicloud', 'vod', fn () => new VodDeployer);
};
