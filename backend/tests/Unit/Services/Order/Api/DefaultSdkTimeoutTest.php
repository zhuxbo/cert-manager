...
test('锁内上游调用带 45s 超时', function (string $method, array $args) {
    ...
    expect($sdk->configs)->toHaveCount(1)
        ->and($sdk->configs[0])->toMatchArray(['connect_timeout' => 3, 'timeout' => 45]);
})->with([
    'new' => ['new', [['refer_id' => 'a']]],
...
test('文档上传带 120s 超时', function () {
    ...
    expect($sdk->configs)->toHaveCount(1)
        ->and($sdk->configs[0])->toMatchArray(['connect_timeout' => 3, 'timeout' => 120]);
});
...
test('其他锁外查询/操作带 30s 超时', function (string $method, array $args) {
    ...
    expect($sdk->configs)->toHaveCount(1)
        ->and($sdk->configs[0])->toMatchArray(['connect_timeout' => 3, 'timeout' => 30]);
})->with([
    'get(sync)' => ['get', [123]],
