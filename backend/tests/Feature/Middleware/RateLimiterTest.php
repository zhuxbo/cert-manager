...
test('RateLimiter IP 限流 - 超过限制抛出异常', function () {
    ...
    // 当前窗口已达限额
    Cache::store('runtime')->put(currentWindowKey($baseKey), 121, 120);
...
test('RateLimiter 默认限流更严格', function () {
    ...
    Cache::store('runtime')->put(currentWindowKey($baseKey), 61, 120);
...
test('RateLimiter 计数器递增正确', function () {
    ...
    expect((int) Cache::store('runtime')->get(currentWindowKey($baseKey)))->toBe(1);
    ...
    expect((int) Cache::store('runtime')->get(currentWindowKey($baseKey)))->toBe(2);
});
...
test('滑动窗口 - 上一窗口计数加权影响当前判定', function () {
    ...
    // 无论何时运行都必定被拒
    Cache::store('runtime')->put(prevWindowKey($baseKey), 7200, 120);
...
test('滑动窗口 - 无上一窗口数据时仅看当前窗口', function () {
    ...
    Cache::store('runtime')->put(currentWindowKey($baseKey), 119, 120);
...
test('滑动窗口 - 当前窗口满载触发限流', function () {
    ...
    Cache::store('runtime')->put(currentWindowKey($baseKey), 120, 120);
...
test('滑动窗口 - 上一窗口少量请求不影响当前窗口正常使用', function () {
    ...
    Cache::store('runtime')->put(prevWindowKey($baseKey), 30, 120);
    Cache::store('runtime')->put(currentWindowKey($baseKey), 80, 120);
...
test('滑动窗口 - 两个窗口累计超限被拒绝', function () {
    ...
    Cache::store('runtime')->put(prevWindowKey($baseKey), 120, 120);
    Cache::store('runtime')->put(currentWindowKey($baseKey), 119, 120);
...
test('滑动窗口 - 不同 limiter 隔离计数', function () {
    ...
    Cache::store('runtime')->put(currentWindowKey($v2Key), 119, 120);
    ...
    expect((int) Cache::store('runtime')->get(currentWindowKey($acmeKey)))->toBe(1);
    expect((int) Cache::store('runtime')->get(currentWindowKey($v2Key)))->toBe(119);
});
...
test('滑动窗口 - key 包含窗口序号', function () {
    ...
    expect(Cache::store('runtime')->has($expectedKey))->toBeTrue();
    expect((int) Cache::store('runtime')->get($expectedKey))->toBe(1);
});
