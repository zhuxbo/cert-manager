...
//   2. 非 HttpResponseException（ValidationException / AuthenticationException 等）—— 走 render callback。
...
test('AuthenticationException 响应中文（未登录或登录已过期）不转义为 unicode', function () {
    // 未带 JWT 访问 user 端点 → AuthenticationException（走 render callback）
    $response = $this->getJson('/api/acme');

    expect($response->getContent())->toContain('未登录或登录已过期');
    expect($response->json('code'))->toBe(0);
});
