...
class NotificationTemplateSeeder extends Seeder
{
    public function run(): void
    {
        $templates = [
            ...
            // 委托失效提醒（delegation:check 周巡检确认委托无效 + 达失败阈值 + 有 active 证书引用其域名）。
            // 强制发（不入 user_default_preferences）；site_url/site_name/hint 由 Builder 从系统设置/常量注入。
            [
                'code' => 'delegation_invalid',
                'name' => '域名委托失效提醒',
                'content' => $this->getDelegationInvalidHtml(),
                'variables' => ['username', 'email', 'delegations'],
                'example' => null,
            ],
            // 安全通知
            [
...
HTML;
     ...
     * @noinspection CssReplaceWithShorthandSafely
     */
    private function getDelegationInvalidHtml(): string
    {
        return <<<'HTML'
<!DOCTYPE html>
<html lang="zh-CN">
...
<body style="margin: 0; padding: 0; background-color: #f4f6f8;">
    ...
    <div style="display: none; font-size: 1px; line-height: 1px; max-height: 0px; max-width: 0px; opacity: 0; overflow: hidden; mso-hide: all; font-family: sans-serif;">
        您的域名 CNAME 委托解析未配置或已失效，自动续期将无法完成验证，请尽快处理。
    </div>
                    ...
                    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" class="white-card" style="max-width: 600px; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.05); text-align: left;">
                        ...
                        <tr>
                            <td class="mobile-padding" style="padding: 40px 40px 30px 40px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
                                ...
                                <div class="notice-box" style="background-color: #fffbeb; border-left: 4px solid #f59e0b; padding: 15px; border-radius: 0 4px 4px 0; margin-bottom: 24px;">
                                    <p class="notice-text" style="margin: 0; font-size: 14px; line-height: 22px; color: #92400e;">
                                        这是一封<strong>委托失效重要提醒</strong>：因涉及自动续期与服务连续性风险，本提醒不受常规到期提醒偏好控制。
                                    </p>
                                </div>
                                ...
                                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" class="data-table" style="margin-bottom: 24px; border-collapse: collapse; width: 100%;">
                                    <thead>
                                        <tr style="background-color: #fffbeb;">
                                            <th align="left" style="padding: 12px 10px; border-bottom: 2px solid #fcd34d; font-size: 13px; font-weight: 600; color: #92400e; text-transform: uppercase;">序号</th>
                                            <th align="left" style="padding: 12px 10px; border-bottom: 2px solid #fcd34d; font-size: 13px; font-weight: 600; color: #92400e; text-transform: uppercase;">委托域名</th>
                                            <th align="left" style="padding: 12px 10px; border-bottom: 2px solid #fcd34d; font-size: 13px; font-weight: 600; color: #92400e; text-transform: uppercase;">CNAME 目标</th>
                                            <th align="center" style="padding: 12px 10px; border-bottom: 2px solid #fcd34d; font-size: 13px; font-weight: 600; color: #92400e; text-transform: uppercase;">连续失败</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td align="left" style="padding: 12px 10px; border-bottom: 1px solid #eeeeee; font-size: 14px; font-weight: 600; color: #333333; font-family: monospace; vertical-align: top;">
                                                {{ $delegation['prefix'] }}.{{ $delegation['zone'] }}
                                            </td>
                                            <td align="left" style="padding: 12px 10px; border-bottom: 1px solid #eeeeee; font-size: 13px; color: #666666; font-family: monospace; vertical-align: top; word-break: break-all;">
                                                {{ $delegation['target_fqdn'] }}
                                            </td>
                                            <td align="center" style="padding: 12px 10px; border-bottom: 1px solid #eeeeee; font-size: 14px; vertical-align: top;">
                                                <span style="background-color: #fee2e2; color: #dc2626; padding: 2px 6px; border-radius: 4px; font-weight: bold; font-size: 12px;">{{ $delegation['fail_count'] }} 次</span>
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                ...
                                <div class="warning-box" style="background-color: #fffbeb; border-left: 4px solid #f59e0b; padding: 15px; border-radius: 0 4px 4px 0; margin-bottom: 30px;">
                                    <p class="warning-text" style="margin: 0; font-size: 14px; line-height: 22px; color: #92400e;">
                                        <strong>重要提示：</strong><br>
                                        请将上表「委托域名」按「CNAME 目标」配置解析。委托长期失效将导致自动续期无法完成域名验证，证书到期后浏览器会显示“不安全”警告。
                                    </p>
                                </div>
