...
class AcmeController extends BaseController
{
     ...
     * 创建 ACME 订单
     *
     * 用户端表单不显示"赠送时间"设置项，默认 1（赠送）。
     * 与传统订单一致，支持通过 URL 查询参数 `?plus=0` 主动取消赠送。
     */
    ...
    {
        $this->action->remark($id, $request->string('remark')->trim()->limit(255), 'remark');
    }
    ...
    /**
     * 批量复制 EAB（UserScope 自动限制仅当前用户；跨用户校验对 User 端是 no-op）
     */
