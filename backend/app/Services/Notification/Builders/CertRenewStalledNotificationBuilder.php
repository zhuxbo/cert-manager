 ...
 *
 * 续费/重签把前驱证书终态化（renewed/reissued）后，接替证书长期卡在非 active 停滞态、前驱即将到期。
...
class CertRenewStalledNotificationBuilder implements NotificationBuilderInterface
{
    public function build(NotificationIntent $intent, Model $notifiable): ?NotificationPayload
    {
        ...
        foreach ($predecessors as $predecessor) {
            ...
            $certificates[] = [
                ...
                'action_hint' => $this->actionHint($successor->status),
            ];
        }
    ...
    /**
     * 拉取该用户「续期停滞」的前驱证书（预载接替 nextCert）。
     *
     * 经 StalledRenewalQuery::forUser 单一形态重查（前驱轴 + EXISTS 接替 5 态 + 48h 门槛 + 连续 14 天超集
     ...
     */
    private function actionHint(string $successorStatus): string
    {
        return match ($successorStatus) {
            ...
            'processing', 'approving' => '新证书正在进行域名验证/审核（费用已扣除）。请尽快完成域名验证或补齐审核材料，以免原证书到期造成服务中断；请勿重复下单或支付。',
