...
- **产品价格初始化只倍乘原始完整成本**：初始化直接读取原始 `cost` JSON，要求当前周期的主成本及所有适用 SAN 成本字段完整；纯主价产品主成本必须大于 0，混合多域名产品允许主成本为 0，但所有适用 SAN 成本必须大于 0。人工保存成本必须先经 `ProductCostNormalizer` 校验并写入完整规范化结果；产品导入对数组成本原样保存、不因成本问题阻断，严格校验统一由价格初始化承担。正式执行不得绕过预览指纹、强制仅选中级别的范围和全局数据库命名锁。详见 `skills/backend/order-fund.md`
