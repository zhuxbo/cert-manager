...
- **插件管理**：`PluginManager` 提供安装/更新/卸载/检查更新，管理端 `/plugin` 页面操作；页面仅展示执行中和失败的插件任务，成功安装/更新只刷新插件列表、不保留成功记录；异步安装/更新任务失败后会阻止同插件继续普通安装/更新，需在失败记录上点「重试」（复用原 operation 重新入队）或「卸载」（清理失败安装记录）后再继续，避免重复堆叠失败任务
...
- **双端管理 + 共享推送 action**：① user/admin 各自鉴权后调共享 `Services\DeployService`（`order_id` 模式按订单推 enabled 目标 / `target_ids` 模式直查不过滤 enabled，两语义；admin `crossUser=true` 走 `withoutGlobalScopes` 跨用户 + **fail-closed 守卫**：空 `target_ids` 或误传 `order_id` 抛错防全量群发）；② admin 端点 `GET /api/admin/cloud-deploy/{target,access,log}` 全功能搜索（quickSearch/结构化筛选/`order_id`·`target_id`·`is_final` 精确）+ `POST .../deploy` 跨用户手动推送（`force` 默认 true）；③ **admin targets `config` 逐键脱敏**：按 `Deployers\Registry` schema 打码 `secret=true` 键、未注册组合先 `hasDeployer` 守卫再 fail-safe（除 `domain` 外全打码、不调 `resolveDeployer`、不 500），`attachUsernames` 把 user_id 拍平为顶层 `username` 供 admin「用户」列；④ `CloudDeployTarget::order()` BelongsTo(`withoutGlobalScopes`) 支撑域名搜索 `whereHas('order.latestCert')`（禁 raw join 防 1052 列名歧义）；⑤ 前端：订单详情「证书部署」单节点 3 块竖条（主系统 `process.vue` ×2 合并下载/自动部署/推送）、user `DeployActions` 全功能（绑定/编辑/推送/记录弹窗，`isActive()` 下沉只禁推送按钮、`isSM2()` 隐藏整块）、admin 云部署菜单（`parent:"System"` + 3 tab 全局视角/用户名列/搜索/脱敏 + widget 只看+推送不绑定）；⑥ `plugin.json` `requires>=0.6.3`（**占位**，发布前按含 process.vue 重构的主系统版本回填）。开发细节见 development.md
