...
- **国密 openssl（统一系统 OpenSSL 3）**：PHP openssl 扩展不支持 SM2，CSR 生成走 `BinaryLocator::gmOpenssl()`（探测 `ecparam -name SM2 -genkey -noout` 验真支持 SM2、防普通 openssl 假阳性）。`gmOpenssl()` 与系统 `openssl()` **同源**——共用系统 openssl 候选路径 + shell 兜底，仅探测条件加严到「真支持 SM2」，**不接独立国密二进制**。**OpenSSL 3.0+ default provider 原生支持 SM2**（LibreSSL/no-sm2/FIPS-only 例外，功能探测会正确拒绝、不签错证书；不支持即 fail-closed 拒下单，绝不静默降级 RSA）；RSA/ECDSA 走 PHP openssl 扩展不受影响。dev 容器、CI（ubuntu-latest）、生产统一靠系统 OpenSSL 3.0，**已彻底移除独立国密二进制的硬编码候选与镜像编译**（勿再加回）
