...
`App\Services\Notification\SystemAlert`：把「运维告警」标准化为一次经 `NotificationCenter` 的 `system_alert` 通知投递，附 Cache 状态指纹去重防持续异常态刷屏。只做传输层，聚合/阈值判定归调用方；异步加密（ShouldBeEncrypted）、afterCommit、`onQueue('notifications')` 全由 `NotificationJob` 继承。消费方：监控命令群、AutoRenew A4 缺价（`missing_price`）、部署失败完整小时聚合（`deploy_failure`）、F2-4 证书链校验（`Order/Action.php:773/793`）、H1 升级 watchdog、H3 备份失败；**P0 批次新增**：M7 上游连通性（`ca_connectivity`/`ca_outage`）、T1 僵尸任务转人工（`task_stale`）、T5 卡单每日快照（`reconcile_maxed`）、T6 ACME 卡单转人工（`acme_reconcile`）；**P3 包W 新增**：委托健康周巡检 DNS 探测系统性停摆（`DelegationCheckCommand`，category `delegation_patrol`、dedupeKey `delegation_patrol_outage`、固定指纹 `patrol_outage`、TTL 504h=3×周巡检周期、未熔断轮 clearDedupe）。`ValidateCommand` 的 dnsTools 全挂只保留 order 级 sync 安全网，不发送管理员告警；传统订单与 ACME 退款失败均不使用专属 `SystemAlert`，由事务回滚、任务失败留痕和每日资金对账兜底；产品导入仅保留人工入口，不使用 `SystemAlert`。**本体三件套（`SystemAlert` / `SystemAlertNotificationBuilder` / `config/notification.builders` 注册）契约冻结，新消费方只适配、不改造本体。**
...
- **恢复清键分两类（选型须知）**：**level/状态监控型**消费方（E1/E3~E6/H3/A4/W 委托巡检，健康态可周期性检测）healthy 分支**必须** `clearDedupe`——否则条件恢复后复发被旧键静默压掉；**窗口聚合或事件驱动告警型**（部署失败小时窗口 / F2-4 chain fail-open / H1 watchdog 自愈）无稳定的「healthy」轮询点，不要求清键：部署聚合靠窗口指纹保证每小时至多一封，chain 恢复即写链使下次 `Chain::exists()` 短路 send 不可达，watchdog 自愈后 status→failed 早返且内容指纹含 pid 新进程必变。
