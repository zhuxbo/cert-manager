...
Redis 编号由 `RedisDatabaseConfig::preserve()` 统一处理：本实例 `.env` 显式设置优先，已加载配置（含 config cache）仅用于确定迁移源及补全缺失项。已有项原位更新、合并重复键，仅追加缺失项，保留属主/权限与 `APP_NAME`。若两个目标编号相同，保留 `REDIS_DB`，扫描 `MANAGER_SITES_ROOT`（默认 `/www/wwwroot`）下同一 Redis 端点的实例占用，并以独立连接确认候选库为空，从 1-15 自动分配 cache DB；分配与写入沿用安装器 flock。后台和脚本规则一致。
