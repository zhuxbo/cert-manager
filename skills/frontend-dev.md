...
- **轮询必须上提到父级**：父级单个 `batchShow` 定时批量刷新（3 分钟 + `visibilitychange` 切回前台立即刷、后台跳过），替代每卡片各自 `setInterval`——否则 N 卡片 = N 并发 `show()` 风暴，切回前台时齐发。`setInterval` + listener 必须在 `onBeforeUnmount` 完整清理。
