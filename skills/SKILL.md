...
| 自动部署上报    | `backend/deploy-renewal.md`                   | auto_deploy_reports 表用途、服务端自写签发失败行（两写入点）、告警消噪（per-order 去重/TTL/持续未解决提醒）、报告清理、Deploy API 边界契约（空查询不分页/续费链判环限跳/errors.error_code）、与客户端契约对应 |
