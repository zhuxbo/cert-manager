...
`platform-config.json` 不再保存版本号、标题、品牌、DNS 工具、备案号、Logo 或二维码。
...
- `qrcode`：用户首页客服二维码，上传时按 1:1 裁剪（输出不超过 800×800、1MB）。

“品牌设置”中的 `admin` 和 `user` 是两个独立的品牌键值对象，格式为 `{ "品牌值": "显示名称" }`，直接使用数组组件已有的“键值对模式”维护。用户端由 `user` 配置生成品牌选项和标签，不保留固定品牌字典。
